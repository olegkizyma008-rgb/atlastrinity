...
async def run():
    nav = await mcp_manager.call_tool('puppeteer','puppeteer_navigate', {'url':'https://accounts.google.com/signup'})
    print('Navigate content:', [(getattr(c,'text',None) or '')[:80] for c in nav.content])
    shot = await mcp_manager.call_tool('puppeteer','puppeteer_screenshot', {'name':'google_signup','encoded':True})
