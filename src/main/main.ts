...
function startPythonServer() {
    ...
    pythonProcess.stdout?.on('data', (data: Buffer) => {
        ...
        console.log(`[Python]: ${message}`);
        if (mainWindow) {
            mainWindow.webContents.executeJavaScript(`console.log('[Python]: ' + ${JSON.stringify(message)})`);
        }
    });
    ...
    pythonProcess.stderr?.on('data', (data: Buffer) => {
        ...
        if (mainWindow) {
            mainWindow.webContents.executeJavaScript(`console.error('[Python Err]: ' + ${JSON.stringify(message)})`);
        }
    });
    ...
    pythonProcess.on('error', (err: Error) => {
        ...
        if (mainWindow) {
            mainWindow.webContents.executeJavaScript(`console.error('CRITICAL: Failed to start Python server: ' + ${JSON.stringify(err.message)})`);
        }
    });
    ...
    pythonProcess.on('close', (code: number | null) => {
        ...
        if (mainWindow && code !== 0 && code !== null) {
            mainWindow.webContents.executeJavaScript(`console.error('CRITICAL: Python server exited with code ' + ${code})`);
        }
    });
}
